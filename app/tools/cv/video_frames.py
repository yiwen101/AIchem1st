...
class ExtractFramesTool(BaseTool):
    """Tool for extracting frames from a video at regular intervals."""
    
    name = "extract_frames"
    description = "Extract frames from a video at regular intervals. Useful for analyzing video content frame by frame."
    parameters = [
        ToolParameter(
            name="video_path",
        ...
        ToolParameter(
            name="output_dir",
            type=ToolParameterType.STRING,
            description="Directory to save frames (if not provided, a directory based on the video name will be created)",
            required=False,
            default=None
        ),
        ToolParameter(
...
@register_cv_tool
class SeparateBackgroundForegroundTool(BaseTool):
    """Tool for separating background and foreground from a video."""
    
    name = "separate_background_foreground"
    description = "Separate background and foreground from a video. Useful for isolating moving objects from static backgrounds."
    parameters = [
        ToolParameter(
            name="video_path",
            type=ToolParameterType.STRING,
        ...
        ToolParameter(
            name="output_dir",
            type=ToolParameterType.STRING,
            description="Directory to save outputs (if not provided, a directory based on the video name will be created)",
            required=False,
            default=None
        ),
        ToolParameter(
            name="sample_frames",
            type=ToolParameterType.INTEGER,
            description="Number of frames to use for learning background",
            required=False,
            default=500
        ),
        ToolParameter(
            ...
            description="Whether to save sample frames from background/foreground",
            required=False,
            default=True
        ),
        ToolParameter(
            name="interval",
            type=ToolParameterType.FLOAT,
            description="Interval in seconds between saved frames",
            required=False,
            default=1.0
        )
    ]
