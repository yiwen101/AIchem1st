...
from app.model.structs import (
    ParquetFileRow,
    QueryVisionLLMResponseWithExplanation,
    VisionModelRequest,
    YoutubeVideoInfo,
)
...
class L3Agent(IVideoAgent):
    ...
    def _preload_video(self, video_id: str) -> bool:
        ...
        try:
            ...
            logger.log_info(
                f"Loaded video {video_id} - Duration: {metadata['duration']:.2f}s, Resolution: {metadata['width']}x{metadata['height']}"
            )
            return True
    ...
    def analyze_full_video(self, query: str) -> QueryVisionLLMResponseFullVideo:
        ...
        frames, _ = self.resource_manager.extract_frames_between(
            num_frames=num_frames, save_frames=False
        )
        ...
        request = VisionModelRequest(
            prompt,
            frames,
            response_class=QueryVisionLLMResponseFullVideo,
            high_detail=self.high_detail,
        )
        try:
            response = query_vision_llm(
                request,
                model=self.model,
            ...
            logger.log_info(
                f"Full video analysis - Explanation: {response.explanation[:100]}..."
            )
            logger.log_info(
                f"Full video analysis - Need zoom in to part of video: {response.need_zoom_in_to_part_of_video}"
            )
            return response
        except Exception as e:
            logger.log_error(f"Error in full video analysis: {str(e)}")
            return (
                f"Error analyzing the video: {str(e)}",
                "An error occurred during analysis.",
            )

    def analyze_segment(
        self, query: str, start_time: float, end_time: float, segment_type: str
    ) -> QueryVisionLLMResponseSegment:
        """
        ...
        """
        logger.log_info(
            f"Analyzing {segment_type} for query: {query} ({start_time:.2f}s to {end_time:.2f}s)"
        )
        ...
        frames, _ = self.resource_manager.extract_frames_between(
            num_frames=num_frames, start_time=start_time, end_time=end_time
        )
        ...
        request = VisionModelRequest(
            prompt,
            frames,
            response_class=QueryVisionLLMResponseSegment,
            high_detail=self.high_detail,
        )
        try:
            response = query_vision_llm(
            ...
            logger.log_info(
                f"{segment_type.capitalize()} analysis - Answer: {response.answer}"
            )
            logger.log_info(
                f"{segment_type.capitalize()} analysis - Explanation: {response.explanation[:100]}..."
            )
            logger.log_info(
                f"{segment_type.capitalize()} analysis - Need zoom in to single image of video: {response.need_zoom_in_to_single_image_of_video}"
            )
            return response
        except Exception as e:
            logger.log_error(f"Error in {segment_type} analysis: {str(e)}")
            return (
                f"Error analyzing the {segment_type}: {str(e)}",
                "An error occurred during analysis.",
            )

    def analyze_key_image(
        self, query: str, image_time: float
    ) -> QueryVisionLLMResponseKeyImage:
        """
        ...
        try:
            ...
            if frame is None or frame.size == 0:
                logger.log_error(
                    f"Failed to extract key image at time {image_time:.2f}s"
                )
            ...
            logger.log_info(
                f"Key image analysis - Explanation: {response.explanation[:100]}..."
            )
            ...
            return (
                f"Error analyzing the key image: {str(e)}",
                "An error occurred during analysis.",
            )
    ...
    def generate_hint_prompt(self, query: str, video_info: YoutubeVideoInfo) -> str:
        ...
        if os.path.exists(file_path):
            with open(file_path, "r") as f:
                queryVideoLLMResponse = QueryVisionLLMResponseHint.model_validate_json(
                    f.read()
                )
        ...
        request = VisionModelRequest(
            prompt, [], response_class=QueryVisionLLMResponseHint
        )
        response = query_vision_llm(
            request,
            model=self.model,
    ...
    def get_answer(self, row: ParquetFileRow) -> str:
        ...
        try:
            ...
            if not relevant_segment_response.need_zoom_in_to_single_image_of_video:
                logger.log_info(
                    "Relevant segment analysis is sufficient, returning answer"
                )
            ...
            if not important_image_result.is_last_scene:
                ...
                if not extended_segment_response.need_zoom_in_to_single_image_of_video:
                    logger.log_info(
                        "Extended segment analysis is sufficient, returning answer"
                    )
            ...
            key_image_response = self.analyze_key_image(
                query, important_image_result.image_time
            )
