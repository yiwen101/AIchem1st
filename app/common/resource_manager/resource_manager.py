...
class ResourceManager:
    ...
    def extract_frames_between(
        ...
        actual_num_frames = min(
            num_frames, int((end_time - start_time) * metadata["fps"])
        )

        if actual_num_frames <= 0:
            raise ValueError(
                f"Cannot extract frames: invalid frame count ({actual_num_frames})"
            )
        ...
        for i, time_point in enumerate(time_points):
            try:
                ...
                if save_frames:
                    self.save_image(
                        frame, time_point, tool_name, suffix=f"_frame_{i + 1}"
                    )
            except Exception as e:
                raise ValueError(
                    f"Error extracting frame at time {time_point:.2f}s: {str(e)}"
                )
    ...
    def get_frame_at_time(self, time_seconds: float) -> Tuple[np.ndarray, int]:
        ...
        if frame_index < 0 or frame_index >= metadata["frame_count"]:
            raise ValueError(
                f"Time {time_seconds}s is out of range for video with duration {metadata['duration']}s"
            )
        ...
        if not ret:
            raise ValueError(
                f"Could not read frame at time {time_seconds}s (frame {frame_index})"
            )
    ...
    def cleanup(self):
        ...
        logger.log_debug("ResourceManager: Released video resources")
