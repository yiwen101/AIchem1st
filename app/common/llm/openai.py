...
def query_llm_structured(
    model: str, messages: List[Dict[str, Any]], response_class: Type[BaseModel]
) -> BaseModel:
    global client
    if client is None:
        client = load_client()
...
def single_query_llm_structured(
    model: str, query: str, response_class: Type[BaseModel]
) -> BaseModel:
...
def query_vision_llm(
    ...
    if display:
        ...
        plt.figtext(
            0.5, 0.02, plot_text, wrap=True, horizontalalignment="center", fontsize=12
        )
...
def query_vision_llm_single_image(
    image: np.ndarray, query: str, model: str = "gpt-4o-mini"
) -> str:
    """
